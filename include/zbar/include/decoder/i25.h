...
typedef struct i25_decoder_s {
    unsigned direction;  /**默认值为1，因为编译不过杨亮取消** scan direction: 0=fwd/space, 1=rev/bar */
    unsigned element; /**默认值为4，因为编译不过杨亮取消** element offset 0-8 */
    int character; /**默认值为12，因为编译不过杨亮取消** character position in symbol */
...
extern void i25_reset (i25_decoder_t *i25);
#if 0
