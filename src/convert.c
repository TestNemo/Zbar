...
/* format definitions */
static zbar_format_def_t format_defs[] = {
...
void uv_round (zbar_image_t *img,
...
void uv_roundup (zbar_image_t *img,
                               const zbar_format_def_t *fmt)
{
    unsigned xmask;
    unsigned ymask;
...
unsigned long uvp_size (const zbar_image_t *img,
                                      const zbar_format_def_t *fmt)
{
    if(fmt->group == ZBAR_FMT_GRAY)
        return(0);
...
uint32_t convert_read_rgb (const uint8_t *srcp,
                                         int bpp)
{
    uint32_t p;
    if(bpp == 3) {
...
void convert_write_rgb (uint8_t *dstp,
                                      uint32_t p,
                                      int bpp)
{
    if(bpp == 3) {
 ...
 */
void convert_y_resize (zbar_image_t *dst,
...
zbar_format_def_t *_zbar_format_lookup (uint32_t fmt)
{
    zbar_format_def_t *def = NULL;
    int i = 0;
                       ...
                       const uint32_t *dsts)
{
    ...
    min_cost = -1;
    for(; *dsts; dsts++) {
        dstfmt = _zbar_format_lookup(*dsts);
        if(!dstfmt)
            continue;
        if(srcfmt->group == dstfmt->group &&
           srcfmt->p.cmp == dstfmt->p.cmp)
